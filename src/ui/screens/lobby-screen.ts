...
export class LobbyScreen implements ScreenController {
  ...
  private changeGroup(dir: number): void {
    ...
    if (this.focus === 0) {
      ...
      this.aiCount = Math.min(this.aiCount, 8 - this.humans);
    } else if (this.focus === 2) {
      const maxAi = 8 - this.humans;
...
private renderHost(): void {
    ...
    const tribeRows = positionTribes(cx, 170 + TITLE_TO_CONTENT + RADIUS, tribeOpts, TRIBE_GAP * 2);
    // Block bottom = last tribe row's bottom (circle + label column).
    const tribeOptionLabelH = tribeLabelH;
  ...
  private renderRoom(): void {
    ...
    const tribeRows = positionTribes(cx, y, tribeOpts, TRIBE_GAP * 2);
