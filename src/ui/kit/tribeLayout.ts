...
export function tribeSlots(count: number, availWidth: number, gap = TRIBE_GAP): { slots: TribeSlot[]; rows: number } {
  const step = TRIBE_DIAM + gap;
  const singleRowW = count * TRIBE_DIAM + (count - 1) * gap;
  let rowSizes: number[];
...
export function positionTribes(cx: number, row1CenterY: number, items: readonly Container[], availWidth: number, gap = TRIBE_GAP): number {
  const { slots, rows } = tribeSlots(items.length, availWidth, gap);
