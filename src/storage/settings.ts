...
interface GameSettings {
  ...
  playerName: string;
  /** True once the user ticked "don't show again" on the game-start welcome. */
...
/** True once the user opted out of the game-start welcome message. */
export function welcomeDismissed(): boolean {
  return loadSettings().welcomeDismissed;
}

export function setWelcomeDismissed(dismissed: boolean): void {
  saveSettings({ ...loadSettings(), welcomeDismissed: dismissed });
}
