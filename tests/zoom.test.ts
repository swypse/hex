...
describe('zoomInertiaStep', () => {
  it('moves the zoom toward the target without overshooting', () => {
    const step = zoomInertiaStep(1.0, 1.2, 1 / 60);
    expect(step.done).toBe(false);
    expect(step.zoom).toBeGreaterThan(1.0);
    expect(step.zoom).toBeLessThan(1.2);
  });

  it('is frame-rate independent: many small steps equal few large steps', () => {
    let a = 1.0;
    for (let i = 0; i < 120; i++) a = zoomInertiaStep(a, 1.2, 1 / 60).zoom;
    let b = 1.0;
    for (let i = 0; i < 60; i++) b = zoomInertiaStep(b, 1.2, 2 / 60).zoom;
    expect(a).toBeCloseTo(b, 6);
  });

  it('snaps to the target and reports done once the gap is tiny', () => {
    const step = zoomInertiaStep(1.2 - 0.0002, 1.2, 1 / 60);
    expect(step.done).toBe(true);
    expect(step.zoom).toBe(1.2);
  });

  it('returns the target unchanged (done) when already there', () => {
    const step = zoomInertiaStep(1.2, 1.2, 1 / 60);
    expect(step.done).toBe(true);
    expect(step.zoom).toBe(1.2);
  });
});

describe('dragLerpStep', () => {
  it('moves the pan partway toward the target, never past or to it instantly', () => {
    const step = dragLerpStep({ x: 0, y: 0 }, { x: 100, y: 50 }, 1 / 60);
    expect(step.done).toBe(false);
    expect(step.pan.x).toBeGreaterThan(0);
    expect(step.pan.x).toBeLessThan(100);
    expect(step.pan.y).toBeGreaterThan(0);
    expect(step.pan.y).toBeLessThan(50);
  });

  it('converges to the target over successive frames and reports done', () => {
    let pan = { x: 0, y: 0 };
    const target = { x: 100, y: 50 };
    let done = false;
    for (let i = 0; i < 400; i++) {
      const step = dragLerpStep(pan, target, 1 / 60);
      pan = step.pan;
      done = step.done;
      if (done) break;
    }
    expect(done).toBe(true);
    expect(pan.x).toBeCloseTo(100, 0);
    expect(pan.y).toBeCloseTo(50, 0);
  });

  it('snaps to a target that is already within epsilon', () => {
    const step = dragLerpStep({ x: 100, y: 50 }, { x: 100.2, y: 50 }, 1 / 60);
    expect(step.done).toBe(true);
    expect(step.pan.x).toBe(100.2);
  });
});
