...
describe('HudToolbar build actions', () => {
  ...
  it('always puts the skills button first in the toolbar', () => {
    const tile = ownedTile(TileType.GrasslandLand);
    selectCell(tile);
    expect(isIconButton(row.children[0] as Container)).toBe(true);

    useGameStore.getState().setSelection(null); // no actions at all
    expect(row.children.length).toBe(2); // skills + end turn
    expect(isIconButton(row.children[0] as Container)).toBe(true);
  });
  ...
  it('shows a Get the bonus icon button when a claimable bonus cell is selected', () => {
    ...
    // The cell shows all applicable actions: the standing unit's disband plus
    // the claimable bonus and the pinned skills/end-turn buttons.
    selectCell(t);
  ...
  it('hides an action the player cannot afford', () => {
    ...
    expect(iconButtons.length).toBe(2); // skills + end turn
  });
