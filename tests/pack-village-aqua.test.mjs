...
import {
  generateVillageAquaAtlas,
  VILLAGE_AQUA_ORDER,
  VILLAGE_AQUA_COLS,
  SOURCE_DIR_URL,
  ATLAS_URL,
} from '../tools/packVillageAqua.mjs';

describe('village-aqua atlas generation', () => {
  ...
  it('covers exactly the PNG files in the village-aqua dir, exactly once', () => {
    expect(VILLAGE_AQUA_ORDER.length).toBe(FILES.length);
    expect([...new Set(VILLAGE_AQUA_ORDER)]).toEqual(VILLAGE_AQUA_ORDER);
    expect(VILLAGE_AQUA_ORDER).toEqual(FILES);
  });

  it('ships the five m/t variants the composite looks up', () => {
    for (const card of ['m1', 'm2', 'm3', 'm4', 't1']) {
      expect(VILLAGE_AQUA_ORDER).toContain(card);
    }
  });

  it('is deterministic: two runs produce identical PNG bytes and manifest', () => {
    const a = generateVillageAquaAtlas(SOURCE_DIR_URL, VILLAGE_AQUA_COLS);
    const b = generateVillageAquaAtlas(SOURCE_DIR_URL, VILLAGE_AQUA_COLS);
  ...
  it('returns non-overlapping 90x90 frames inside the atlas bounds', () => {
    const { frames, width, height } = generateVillageAquaAtlas(SOURCE_DIR_URL, VILLAGE_AQUA_COLS);
    const rows = Math.ceil(FILES.length / VILLAGE_AQUA_COLS);
    expect(width).toBe(VILLAGE_AQUA_COLS * CELL_W);
  ...
  it('keeps the committed atlas PNG in sync with the packer output', async () => {
    ...
    const final = await finalizeAtlas(generateVillageAquaAtlas(SOURCE_DIR_URL, VILLAGE_AQUA_COLS).png);
    expect(atlasPngsMatch(committed, final)).toBe(true);
  });

  it('references only real, valid 90x90 village-aqua PNGs', () => {
    const { frames } = generateVillageAquaAtlas(SOURCE_DIR_URL, VILLAGE_AQUA_COLS);
    for (const id of FILES) {
      const f = frames[id];
      expect(f).not.toBeUndefined();
