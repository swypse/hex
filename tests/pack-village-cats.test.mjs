...
import {
  generateVillageCatsAtlas,
  VILLAGE_CATS_ORDER,
  VILLAGE_CATS_COLS,
  SOURCE_DIR_URL,
  ATLAS_URL,
} from '../tools/packVillageCats.mjs';

describe('village-cats atlas generation', () => {
  ...
  it('covers exactly the PNG files in the village-cats dir, exactly once', () => {
    expect(VILLAGE_CATS_ORDER.length).toBe(FILES.length);
    expect([...new Set(VILLAGE_CATS_ORDER)]).toEqual(VILLAGE_CATS_ORDER);
    expect(VILLAGE_CATS_ORDER).toEqual(FILES);
  });

  it('ships the five m/t variants the composite looks up', () => {
    for (const card of ['m1', 'm2', 'm3', 'm4', 't1']) {
      expect(VILLAGE_CATS_ORDER).toContain(card);
    }
  });

  it('is deterministic: two runs produce identical PNG bytes and manifest', () => {
    const a = generateVillageCatsAtlas(SOURCE_DIR_URL, VILLAGE_CATS_COLS);
    const b = generateVillageCatsAtlas(SOURCE_DIR_URL, VILLAGE_CATS_COLS);
  ...
  it('returns non-overlapping 90x90 frames inside the atlas bounds', () => {
    const { frames, width, height } = generateVillageCatsAtlas(SOURCE_DIR_URL, VILLAGE_CATS_COLS);
    const rows = Math.ceil(FILES.length / VILLAGE_CATS_COLS);
    expect(width).toBe(VILLAGE_CATS_COLS * CELL_W);
  ...
  it('keeps the committed atlas PNG in sync with the packer output', async () => {
    ...
    const final = await finalizeAtlas(generateVillageCatsAtlas(SOURCE_DIR_URL, VILLAGE_CATS_COLS).png);
    expect(atlasPngsMatch(committed, final)).toBe(true);
  });

  it('references only real, valid 90x90 village-cats PNGs', () => {
    const { frames } = generateVillageCatsAtlas(SOURCE_DIR_URL, VILLAGE_CATS_COLS);
    for (const id of FILES) {
      const f = frames[id];
      expect(f).not.toBeUndefined();
