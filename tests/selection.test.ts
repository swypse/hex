...
describe('reachableTargets', () => {
  ...
  it('a land unit may dock on its own port but never cross it to the far shore', () => {
    ...
    const keys = (canDock: boolean) =>
      reachableTargets(map, unit, 40, false, canDock).map((t) => `${t.q},${t.r}`);
    // Docking: the port itself stays reachable as the move's end.
    expect(keys(true)).toContain('1,0');
    // But the port is not a bridge: the far shore is unreachable.
    expect(keys(true)).not.toContain('2,0');
  });
...
describe('pathBetween', () => {
  ...
  it('does not route a land unit through its own port to the far shore', () => {
    ...
    expect(pathBetween(map, { q: 0, r: 0 }, { q: 1, r: 0 }, false, false, true, 0)).toEqual([
      { q: 1, r: 0 },
    ]);
    // ...but crossing it like a bridge is not.
    expect(pathBetween(map, { q: 0, r: 0 }, { q: 2, r: 0 }, false, false, true, 0)).toEqual([]);
  });
  ...
  it('a ship can still sail out of its own port', () => {
    ...
    port.unit = ship;
    expect(pathBetween(map, { q: 0, r: 0 }, { q: 1, r: 0 }, false, true, true, 0)).toEqual([
      { q: 1, r: 0 },
    ]);
    expect(reachableTargets(map, ship, 20).map((t) => `${t.q},${t.r}`)).toContain('1,0');
  });
