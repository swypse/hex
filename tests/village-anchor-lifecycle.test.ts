...
describe('village texture anchor across lifecycle', () => {
  it('keeps the free village anchor whether free or owned (no composite service)', () => {
    ...
    // Render as a free village: anchor should come from the free village texture.
    ...
    // Capture it (owner set): without a composite build service the free
    // village texture still supplies the anchor.
    settlement.owner = 0;
    view.update(map, pl, null, new Set(), new Set(), 0, new Set(), viewport);
    sprite = villageSprite(view, tile)!;
    expect(sprite.anchor.y).toBe(textures.freeVillageTexture.anchorY);

    // Upgrade to level 2: still the free village anchor.
    settlement.level = 2;
    view.update(map, pl, null, new Set(), new Set(), 0, new Set(), viewport);
    sprite = villageSprite(view, tile)!;
    expect(sprite.anchor.y).toBe(textures.freeVillageTexture.anchorY);
