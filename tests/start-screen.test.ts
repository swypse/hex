...
describe('StartScreen background images', () => {
  ...
  it('keeps the menu column static before and after the logo image loads', () => {
    const screen = new StartScreen();
    screen.mount(host);
    const firstButton = (screen as unknown as { buttons: { children: { text?: string }[]; position: { x: number; y: number } }[] }).buttons[0]!;
    const before = firstButton.position.y;

    // Simulate the logo finishing loading (with its real 194x170 source aspect).
    const load = FakeImage.instances.find((i) => i.src.includes('hex-terra.png'))!;
    (load as unknown as { naturalWidth: number; naturalHeight: number }).naturalWidth = 194;
    (load as unknown as { naturalWidth: number; naturalHeight: number }).naturalHeight = 170;
    load.onload?.call(load as never);

    expect(firstButton.position.y).toBeCloseTo(before, 5);
    screen.destroy();
  });
});
