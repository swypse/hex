...
// screen-scroll content group introduced to make overflowing screens scroll.
function uiNodes(screen: LobbyScreen): Container[] {
...
// The picker always draws the tribes as two centered rows (top row gets the
// larger share). Within a row the 88px step comes from TRIBE_GAP * 2 (32px gap
// + 56px circle); the two rows sit TRIBE_ROW_STEP (88px) apart.
function expectTribeGridTwoRows(options: Container[]): void {
  expect(options).toHaveLength(TRIBES.length);
  const rowYs = [...new Set(options.map((o) => Math.round(o.position.y)))].sort((a, b) => a - b);
  expect(rowYs).toHaveLength(2);
  expect(rowYs[1]! - rowYs[0]!).toBeCloseTo(88, 5);
  for (const ry of rowYs) {
    const row = options.filter((o) => Math.round(o.position.y) === ry);
    const xs = row.map((o) => o.position.x).sort((a, b) => a - b);
...
describe('LobbyScreen host keyboard navigation', () => {
  ...
  it('changes the value within the focused group with left/right', () => {
    ...
    const aiOpts = Array.from({ length: 8 - state().humans }, (_, i) => i);
    key('ArrowRight');
  ...
  it('requires at least 2 human players in multiplayer', () => {
    ...
    // Human players is the ButtonGroup whose buttons are the digits 2..7.
    const humanGroup = uiNodes(screen).find((c) => {
      ...
      return digits.length === 6 && digits.includes('2') && digits.includes('7');
    });
    ...
    const digits = humanGroup!.children
      ...
      .filter((t): t is string => t !== undefined)
      .sort();
    expect(digits).toEqual(['2', '3', '4', '5', '6', '7']);
  });
  ...
  it('places the back button as the last navigable button below create room', () => {
    const nodes = uiNodes(screen);
    const create = nodes.find((c) => hasDirectText(c, 'Create room')) as Container;
    const back = nodes.find((c) => hasDirectText(c, 'Back')) as Container;
  ...
  it('centers the create room button', () => {
    const create = uiNodes(screen).find((c) => hasDirectText(c, 'Create room')) as Container;
    expect(create).toBeDefined();
    expect(create.position.x).toBe(1280 / 2 - 120);
  });

  it('renders the tribe choice as circular options with icons', () => {
    const options = uiNodes(screen).filter((c) =>
  ...
  it('spaces the tribe options like the Choose your tribe screen', () => {
    ...
    expectTribeGridTwoRows(options as unknown as Container[]);
  });
  ...
  it('centers the human player and AI opponent buttons', () => {
    ...
    expect(humanGroup.children.length).toBe(6);
    expect(aiGroup.children.length).toBe(6);
...
describe('LobbyScreen menu keyboard navigation', () => {
  ...
  it('places the menu back button as the last button below the others', () => {
    const back = uiNodes(screen).find((c) => hasDirectText(c, 'Back')) as Container;
    expect(back).toBeDefined();
    const cx = 1280 / 2;
...
describe('LobbyScreen client room tribe selection', () => {
  ...
  const optionFor = (tribeName: string): Container =>
    uiNodes(screen).find((c) => hasDirectText(c, tribeName)) as Container;
  ...
  it('lays out all tribe circles like the single-player picker', () => {
    setClientRoom();
    const options = uiNodes(screen).filter((c) =>
      (c as Container).children.some((ch) => ch instanceof Sprite && (ch as { mask: unknown }).mask !== null),
    );
