...
describe('pirate capture messages', () => {
  beforeEach(() => {
    useGameStore.setState({ centerMessage: null, centerMessageQueue: [] });
  });
  ...
  it('reports a captured ship to its owner', async () => {
  ...
  it('reports a failed capture attempt to the ship owner', async () => {
    const owner = player(0, Tribe.Cats);
    useGameStore.setState({ localPlayerIndex: 0, players: [owner] });
    const presenter = new EventPresenter(makeHost([owner]));
    await presenter.present(
