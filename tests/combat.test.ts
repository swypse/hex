...
function catapultMap(): GameMap {
  const catapult = makeCatapult('c', 0, 0, 0, 30);
  const enemyVillage = makeTile(2, 0, TileType.GrasslandLand);
  ...
  friendlyVillage.ownedBy = 0;
  const empty = makeTile(0, 1, TileType.GrasslandLand);
  return { radius: 4, tiles: [makeTile(0, 0, TileType.GrasslandLand, catapult), enemyVillage, enemyMine, enemyBridge, friendlyVillage, empty], spawns: [] };
}
...
describe('resolveCombat', () => {
  it('applies the force-ratio formula with COMBAT_SCALE 1.5', () => {
    const map = makeMap();
...
describe('catapult siege targets', () => {
  it('lists enemy villages, buildings and bridges within range for a catapult', () => {
    const map = catapultMap();
    const catapult = map.tiles[0]!.unit!;
    const keys = attackableTargets(map, catapult, 0).map((t) => `${t.q},${t.r}`);
    expect(keys).toContain('2,0'); // enemy village
    expect(keys).toContain('3,0'); // enemy mine (range 3)
    expect(keys).toContain('1,-1'); // enemy bridge
    expect(keys).not.toContain('-1,0'); // friendly village
    expect(keys).not.toContain('0,1'); // empty tile
  });
  ...
  it('does not list siege tiles carrying an enemy unit (unit attack takes over)', () => {
    const map = catapultMap();
    map.tiles[1]!.unit = makeWarrior('g', 1, 2, 0, 20);
    const catapult = map.tiles[0]!.unit!;
    const keys = attackableTargets(map, catapult, 0).map((t) => `${t.q},${t.r}`);
    expect(keys).toContain('2,0'); // still attackable, via the unit
    expect(keys).toContain('3,0');
    expect(keys).toContain('1,-1');
  });
});

describe('performSiege', () => {
  const alwaysMiss = (): number => 0;

  it('marks the catapult as attacked even on a miss', () => {
    const c = makeCatapult('c', 0, 0, 0, 30);
    const village = makeTile(2, 0, TileType.GrasslandLand);
    village.settlement = { owner: 1, level: 2, captureReady: false };
    village.ownedBy = 1;
    const out = performSiege(c, village, alwaysMiss);
    expect(out.missed).toBe(true);
    expect(out.destroyed).toBeNull();
    expect(c.hasAttacked).toBe(true);
    expect(village.settlement!.level).toBe(2);
  });

  it('destroys a wall first, then downgrades the village level (min 1)', () => {
    const c = makeCatapult('c', 0, 0, 0, 30);
    const village = makeTile(2, 0, TileType.GrasslandLand);
  ...
  it('damages a building by 1 per hit and removes it at 0 hp', () => {
    ...
    expect(first.destroyed).toBeNull();
    expect(first.buildingHp).toBe(1);
  ...
  it('never affects friendly or unowned structures', () => {
    const c = makeCatapult('c', 0, 0, 0, 30);
    const friendly = makeTile(-1, 0, TileType.GrasslandLand);
    friendly.settlement = { owner: 0, level: 2, captureReady: false };
    friendly.ownedBy = 0;
    const out = performSiege(c, friendly, noMiss);
    expect(out.destroyed).toBeNull();
    expect(friendly.settlement!.level).toBe(2);
    expect(isEnemySiegeTarget(friendly, 0)).toBe(false);
  });
});
...
describe('chooseBestAttack', () => {
  ...
  it('a catapult with no unit targets prefers a reachable enemy village over a mine', () => {
    const map = catapultMap();
    const catapult = map.tiles[0]!.unit!;
    const best = chooseBestAttack(map, catapult, 0);
    // Village (2,0) scores higher than the mine (3,0) or bridge (1,-1).
    expect(best?.q).toBe(2);
    expect(best?.r).toBe(0);
  });

  it('a catapult with both an enemy unit and a village target attacks the unit', () => {
    const map = catapultMap();
    map.tiles[1]!.unit = makeWarrior('g', 1, 2, 0, 20);
    const catapult = map.tiles[0]!.unit!;
    const best = chooseBestAttack(map, catapult, 0);
