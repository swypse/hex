...
describe('combat animation ordering', () => {
  ...
  it('shoots an arrow sprite from the archer toward the target and removes it on arrival', async () => {
    ...
    const findArrow = (): Sprite =>
      h.mapView.container.children.find((c) => c instanceof Sprite && c !== null) as Sprite;
    ...
    // The arrow launches from the attacker's hex in map space.
    const launch = hexToPixel({ q: 0, r: 0 }, 40);
    expect(start.x).toBe(launch.x);
    expect(start.y).toBeCloseTo(launch.y - tileElevation(unitAt(map, 0, 0), 40), 0);
...
h.advanceTicks(100);
  ...
  it('keeps the arrow bound to the map when the camera pans mid-flight', async () => {
    const map = makeOpenMap();
    const players = [player(0, Tribe.Cats), player(1, Tribe.Barbarians)];
    h = setup(map, players);
    ...
    await waitFor(() => findArrow() !== undefined);
    // The projectile lives inside the camera-transformed map container, never
    // in the raw (screen-space) map root.
    expect(h.mapRoot.children).not.toContain(findArrow());
    // Its stored position is a world coordinate, independent of the camera:
    // a mid-flight pan must not bend the stored trajectory.
    const before = { x: findArrow().position.x, y: findArrow().position.y };
    (h.gc.camera as { pan: { x: number; y: number } }).pan = { x: 900, y: 700 };
    expect(findArrow().position.x).toBe(before.x);
    expect(findArrow().position.y).toBe(before.y);
  ...
  it('flies the arrow ~150ms per tile of distance (1 tile)', async () => {
    ...
    const p = h.gc.presentEvents([attack], h.gc.exploredKeysFor(0));
    const findArrow = (): Sprite =>
      h.mapView.container.children.find((c) => c instanceof Sprite && (c as Sprite).texture.width === 67) as Sprite;
  ...
  it('scales the cannonball flight with distance (2 tiles ~300ms)', async () => {
    ...
    const findCannonball = (): Sprite =>
      h.mapView.container.children.find((c) => c instanceof Sprite && (c as Sprite).texture.width === 35) as Sprite;
  ...
  it('keeps counter cannonballs sequential after the attack shot lands', async () => {
    ...
    const p = h.gc.presentEvents([attack], h.gc.exploredKeysFor(0));
    const balls = (): number =>
      h.mapView.container.children.filter((c) => c instanceof Sprite && (c as Sprite).texture.width === 35).length;
  ...
  it('shoots a cannonball sprite from the ship toward the target and removes it on arrival', async () => {
    ...
    // The cannonball launches from the ship's hex in map space.
    expect(start.x).toBe(hexToPixel({ q: 0, r: 0 }, 40).x);
