...
import {
  generateVillageSandAtlas,
  VILLAGE_SAND_ORDER,
  VILLAGE_SAND_COLS,
  SOURCE_DIR_URL,
  ATLAS_URL,
} from '../tools/packVillageSand.mjs';

describe('village-sand atlas generation', () => {
  ...
  it('covers exactly the PNG files in the village-sand dir, exactly once', () => {
    expect(VILLAGE_SAND_ORDER.length).toBe(FILES.length);
    expect([...new Set(VILLAGE_SAND_ORDER)]).toEqual(VILLAGE_SAND_ORDER);
    expect(VILLAGE_SAND_ORDER).toEqual(FILES);
  });

  it('ships the five m/t variants the composite looks up', () => {
    for (const card of ['m1', 'm2', 'm3', 'm4', 't1']) {
      expect(VILLAGE_SAND_ORDER).toContain(card);
    }
  });

  it('is deterministic: two runs produce identical PNG bytes and manifest', () => {
    const a = generateVillageSandAtlas(SOURCE_DIR_URL, VILLAGE_SAND_COLS);
    const b = generateVillageSandAtlas(SOURCE_DIR_URL, VILLAGE_SAND_COLS);
  ...
  it('returns non-overlapping 90x90 frames inside the atlas bounds', () => {
    const { frames, width, height } = generateVillageSandAtlas(SOURCE_DIR_URL, VILLAGE_SAND_COLS);
    const rows = Math.ceil(FILES.length / VILLAGE_SAND_COLS);
    expect(width).toBe(VILLAGE_SAND_COLS * CELL_W);
  ...
  it('keeps the committed atlas PNG in sync with the packer output', async () => {
    ...
    const final = await finalizeAtlas(generateVillageSandAtlas(SOURCE_DIR_URL, VILLAGE_SAND_COLS).png);
    expect(atlasPngsMatch(committed, final)).toBe(true);
  });

  it('references only real, valid 90x90 village-sand PNGs', () => {
    const { frames } = generateVillageSandAtlas(SOURCE_DIR_URL, VILLAGE_SAND_COLS);
    for (const id of FILES) {
      const f = frames[id];
      expect(f).not.toBeUndefined();
