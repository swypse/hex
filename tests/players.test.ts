...
describe('buildMultiplayerPlayers', () => {
  ...
  it('throws for invalid totals', () => {
    ...
    expect(() => buildMultiplayerPlayers([{ name: 'A', tribe: Tribe.Villagers }], 7, new SeededRandom(1))).toThrow();
  });

  it('supports up to 7 players total', () => {
    const players = buildMultiplayerPlayers(
      [
    ...
    expect(new Set(players.map((p) => p.tribe)).size).toBe(7);
  });
...
describe('buildPlayers', () => {
  ...
  it('throws for invalid enemy counts', () => {
    ...
    expect(() => buildPlayers(Tribe.Villagers, 7, new SeededRandom(42))).toThrow();
  });

  it('creates 7 players with distinct tribes for 6 enemies', () => {
    const players = buildPlayers(Tribe.Warriors, 6, new SeededRandom(42));
    expect(players).toHaveLength(7);
    expect(new Set(players.map((p) => p.tribe)).size).toBe(7);
  });
